...
void init()
{
...
#if defined(TCCR2A) && defined(WGM01)
	sbi(TCCR2A, WGM21);
	sbi(TCCR2A, WGM20);
#endif  
	...
	sbi(TCCR0B, CS02);
#else
	#warning Timer 0 not finished (may not be present on this CPU)
#endif

	// configurse timer 0 for phase correct pwm (8-bit)
